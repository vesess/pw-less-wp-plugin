 ...
 */
class Vesesslabs_Vesessauth_Security {
    ...
    public function is_ip_blocked($ip_address) {
        $blocked_ips = get_transient('vesesslabs_vesessauth_blocked_ips');
        ...
        if (isset($blocked_ips[$ip_address])) {
            ...
            set_transient('vesesslabs_vesessauth_blocked_ips', $blocked_ips, DAY_IN_SECONDS);
        }
    ...
    public function record_login_attempt($ip_address, $email, $success = false) {
        ...
        $attempts = get_transient('vesesslabs_vesessauth_login_attempts');
        if (!$attempts) {
            $attempts = array();
        }
        ...
        if ($success) {
            ...
            set_transient('vesesslabs_vesessauth_login_attempts', $attempts, self::ATTEMPT_WINDOW);
            return false;
        }
    ...
    public function record_login_request($ip_address, $email) {
        ...
        $requests = get_transient('vesesslabs_vesessauth_login_requests');
        if (!$requests) {
            $requests = array();
        }
        ...
        // Save requests
        set_transient('vesesslabs_vesessauth_login_requests', $requests, self::ATTEMPT_WINDOW);
        return false;
    }
    ...
    public function record_registration_attempt($ip_address) {
        ...
        $attempts = get_transient('vesesslabs_vesessauth_registration_attempts');
        if (!$attempts) {
            $attempts = array();
        }
        ...
        // Save attempts
        set_transient('vesesslabs_vesessauth_registration_attempts', $attempts, self::ATTEMPT_WINDOW);
        return false;
    }
    ...
    private function block_ip($ip_address) {
        ...
        vesesslabs_vesessauth_log("IP address {$ip_address} has been blocked for " . (self::LOCKOUT_DURATION / 60) . " minutes", 'warning');
    }
    ...
    public function cleanup_old_attempts() {
        ...
        $reg_attempts = get_transient('vesesslabs_vesessauth_registration_attempts');
        if ($reg_attempts) {
            ...
            set_transient('vesesslabs_vesessauth_registration_attempts', $reg_attempts, self::ATTEMPT_WINDOW);
        }
